...
let filters = {}, sort = 'price', view = 'catalogue', budgetId = 'moken10', estimates = {}, owned = [], excluded = [], shippingQuotes = {}, extras = 250, budgetChosen = false, budgetFavoritesOnly = false;
...
function budgetPicker() {
  const options = budgetFavoritesOnly ? models.filter(model => favorites.includes(model.id)) : models;
  const currentVisible = options.some(model => model.id === budgetId);
  const placeholder = currentVisible ? '' : `<option value="" selected disabled>${options.length ? 'Choisir parmi tes favoris' : 'Aucun favori pour le moment'}</option>`;
  return `<div class="budget-picker"><label class="check budget-favorites"><input id="budget-favorites" type="checkbox" ${budgetFavoritesOnly ? 'checked' : ''}>${icon('heart')}Mes favoris uniquement (${favorites.length})</label><label>Kayak et pack à chiffrer<select id="budget-model" ${options.length ? '' : 'disabled'} ${currentVisible ? '' : 'aria-describedby="budget-filter-note"'}>${placeholder}${selectOptions(options.map(model => [model.id, `${model.brand} · ${model.name} · ${money(model.price)}`]), currentVisible ? budgetId : '')}</select></label>${currentVisible ? '' : `<p id="budget-filter-note" class="field-hint" role="status">Budget affiché inchangé : ${escape(byId(budgetId).name)} (hors favoris).${options.length ? '' : ' <a href="#catalogue">Choisir des favoris dans les kayaks</a>'}</p>`}</div>`;
}
function budgetView() {
...
function saveBudget() { save('budget', { budgetId, estimates, owned, excluded, shippingQuotes, extras, budgetChosen, budgetFavoritesOnly }); projectResult = null; }
...
document.addEventListener('click', event => {
  ...
  if (action === 'clear-local') { for (const key of ['selection', 'favorites', 'budget', 'profile']) { try { localStorage.removeItem(`rivage.${key}`); } catch {} } selected = []; favorites = []; favoritesOnly = false; filters = {}; estimates = {}; owned = []; excluded = []; shippingQuotes = {}; extras = 250; budgetId = 'moken10'; budgetChosen = false; budgetFavoritesOnly = false; profile = { ...defaultProfile }; projectResult = null; if (view === 'projet') projectView(); renderSelection(); notify('Ton projet et tes favoris ont été effacés sur cet appareil.'); }
...
try {
  ...
  try {
    ...
    if (saved && typeof saved === 'object') {
      ...
      budgetFavoritesOnly = saved.budgetFavoritesOnly === true;
